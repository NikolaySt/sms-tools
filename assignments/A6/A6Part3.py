...
def estimateInharmonicity(inputFile='../../sounds/piano.wav',
    ...
    timestamps = np.arange(xhmag.shape[0]) * H / float(fs)
    frames = np.squeeze(np.where((timestamps >= t1) & (timestamps <= t2)))
    ...
    for index in frames:
        ...
        for r in range(1, R + 1, 1):
            sum_frame = sum_frame + abs(xhfreq[index][r - 1] -
                                        r * f0[index]) / r
        I.append((1 / R) * sum_frame)

    last_index = frames[len(frames) - 1]
    first_index = frames[0]
    I_mean = np.sum(I) / (last_index - first_index + 1)

    return I_mean


def testCase():
    i_mean = estimateInharmonicity(inputFile='../../sounds/piano.wav',
                                   t1=2.55,
                                   t2=2.8,
                                   window='hamming',
                                   M=2047,
