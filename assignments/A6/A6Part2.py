...
def segmentStableNotesRegions(inputFile='../../sounds/sax-phrase-short.wav',
    ...
    stds = np.zeros(f0c.shape[0])
    for i in range(f0c.shape[0]):
        if i < winStable - 1:
            continue
        elements = f0c[i - winStable + 1:i + 1]
        std = np.std(elements)
        stds[i] = std

    #3. apply threshold on standard deviation values to find indexes of the stable points in melody
    stabenoteIndexes = np.squeeze(np.where(stds < stdThsld))
    #4. create segments of continuous stable points such that consecutive stable points belong to same segment
    segments = []
    segment = []
    segments.append(segment)
    for i in range(stabenoteIndexes.shape[0]):
        if i == 0:
            continue
        index_prev = stabenoteIndexes[i - 1]
        index_curr = stabenoteIndexes[i]
        if (index_curr == index_prev + 1):
            segment.append(index_curr)
        else:
            segment = []
            segment.append(index_curr)
            segments.append(segment)
    ...
    filteredSegments = []
    timestamps = np.arange(f0.shape[0]) * H / float(fs)
    for i in range(len(segments)):
        last_index = segments[i][len(segments[i]) - 1]
        fist_index = segments[i][0]

        duration = timestamps[last_index] - timestamps[fist_index]
        if duration < minNoteDur:
            continue

        filteredSegments.append([fist_index, last_index])

    filteredSegments = np.array(filteredSegments)
    #plotSpectogramF0Segments(
    #    x, fs, w, N, H, f0,
    #    filteredSegments)  # Plot spectrogram and F0 if needed
...
def plotSpectogramF0Segments(x, fs, w, N, H, f0, segments):
    ...
    for ii in range(segments.shape[0]):
        plt.plot(timeStamps[segments[ii, 0]:segments[ii, 1]],
                 f0[segments[ii, 0]:segments[ii, 1]],
                 color='#A9E2F3',
                 linewidth=1.5)
