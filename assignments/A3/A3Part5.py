...
def get_anal(x, N):
    ...
    X[:hN].real[np.abs(X[:hN].real) < EPSILON] = 0.0
    # for phase calculation set to 0 the small values
    X[:hN].imag[np.abs(X[:hN].imag) < EPSILON] = 0.0
...
def zpFFTsizeExpt(x, fs):
    ...
    norm_w2 = w2 / sum(w2)
    # Your code here
    mX1, ph1 = get_anal(xseg * norm_w1, 256)
    mX2, ph2 = get_anal(x * norm_w2, 512)
    mX3, ph3 = get_anal(xseg * norm_w1, 512)
    #mX1, ph1 = dftAnal(xseg, w1, 256)
    #mX2, ph2 = dftAnal(x, w2, 512)
    #mX3, ph3 = dftAnal(xseg, w1, 512)

    bin_freq1 = np.arange(0, 80, 1)
    bin_freq2 = np.arange(0, 80, 1)
    for i in range(80):
        bin_freq1[i] = fs*i/256
        bin_freq2[i] = fs*i/512

    return (mX1[:80], bin_freq1), (mX2[:80], bin_freq2), (mX3[:80], bin_freq2)
...
def testCase():
    ...
    import matplotlib.pyplot as plt
    (m1, binf1), (m2, binf2), (m3, binf3) = zpFFTsizeExpt(x[:512], fs)
    plt.plot(binf1, m1, label="256 samples, w1 256, FFT 256")
    plt.plot(binf2, m2, label="512 samples, w2 512, FFT 512")
    plt.plot(binf3, m3, label="256 samples, w1 256, FFT 512")
    plt.legend()
    plt.xlabel("bins")
    plt.ylabel("-dB")
    plt.show()
