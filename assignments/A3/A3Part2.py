...
def optimalZeropad(x, fs, f):
    ...
    import math
    M = x.shape[0]
    period_samples = float(fs) / float(f)
    N = int(M // period_samples * period_samples)
    if M - N > 0:
        N = int((M // period_samples + 1) * period_samples)
...
def test_case():
    ...
    #dict1 = loadTestCases.load(1, 1)
    #x = dict1['input']['x']
    # fs = dict1['input']['fs']
    # f1 = dict1['input']['f1']
    # f2 = dict1['input']['f2']
    #mX = minimizeEnergySpreadDFT(x, fs, f1, f2)
    mX = optimalZeropad(x[:M], fs, f)
    print(mX.shape)
    plt.plot(mX)
    plt.show()
