...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    # plt.title(f'mX ({inputFile}), M={M}, N={N}, H={H}')
    # plt.autoscale(tight=True)

    # plt.subplot(212)
    # plt.plot(frmTime, engEnv[:, 0], 'b')
    # plt.plot(frmTime, engEnv[:, 1], 'r')
...
def testCase():
    ...
    dict1 = loadTestCases.load(3, 1)
    output = dict1['output']

    engEnv = computeEngEnv(
        '../../sounds/piano.wav', 'blackman', M=513, N=1024, H=128)

    # engEnv = computeEngEnv(
    #    '../../sounds/piano.wav', 'blackman', M = 2047, N = 4096, H = 128)

    # engEnv = computeEngEnv(
    #   '../../sounds/sax-phrase-short.wav', 'hamming', M = 513, N = 2048, H = 256)
