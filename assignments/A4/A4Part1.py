...
def extractMainLobe(window, M):
    ...
    mX_zero_centered = np.zeros(N)
    mX_zero_centered[-N//2:] = mX[:N//2]
    mX_zero_centered[:N//2] = mX[-N//2:]

    max_index = np.argmax(mX_zero_centered)
    min_lobe_index = -1
    index = max_index
    while(True):
        index = index - 1
        if mX_zero_centered[index] > mX_zero_centered[index + 1]:
            min_lobe_index = index + 1
            break

    return mX_zero_centered[min_lobe_index:max_index + (max_index - min_lobe_index) + 1]


def testCase():
    mX = extractMainLobe("hamming", 256)
    print(mX.shape)
    plt.plot(mX)
    plt.show()
