...
def computeSNR(inputFile, window, M, N, H):
    ...
    xE = np.sum(np.square(x))
    yE = np.sum(np.square(x - y))

    snr1 = 10 * np.log10(xE/(yE))

    xE = np.sum(np.square(x[M:-M]))
    yE = np.sum(np.square(x[M:-M] - y[M:-M]))    

    snr2 = 10 * np.log10(xE/yE)

    return snr1, snr2


def testCase():
    snr1, snr2 = computeSNR('../../sounds/sax-phrase-short.wav',
                             "hamming", 512, 1024, 64)
    print(snr1, snr2)
