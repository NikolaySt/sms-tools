...
def descriptorPairScatterPlot(inputDir, descInput=(0, 0), anotOn=0):
  ...
  """
    if max(descInput) >= len(descriptorMapping.keys()):
        print(
            "Please select a descriptor index that is within the range. Maximum descriptor index can be "
            + str(len(descriptorMapping) - 1) +
            ". Check the descriptor index mapping again using function "
    ...
    for ii, category in enumerate(dataDetails.keys()):
        ...
        for soundId in dataDetails[category].keys():
            filepath = os.path.join(inputDir, category, soundId,
                                    dataDetails[category][soundId]['file'])
            ...
            if anotOn == 1:
                plt.annotate(soundId,
                             xy=(x_cord, y_cord),
                             xytext=(x_cord, y_cord))
    ...
    #plt.show()
    fchart  = f"{descInput[0]}_{descInput[1]}.png"
    plt.savefig(fchart)
...
def convFtrDict2List(ftrDict):
  ...
  """
    ...
    for key in range(len(descriptorMapping.keys())):
        try:
            ftrName, ind = '.'.join(
                descriptorMapping[key].split('.')[:-1]), str(
                    descriptorMapping[key].split('.')[-1])
  ...
  """
    ...
    for cname in dataDetails.keys():
        ...
        for sname in dataDetails[cname].keys():
            eucDist = eucDistFeatures(qFtr,
                                      dataDetails[cname][sname]['feature'],
                                      descInput)
...
def classifySoundkNN(queryFile, targetDir, K, descInput=[]):
  ...
  """
    ...
    if len(np.where(
        (np.array(distances)[:, 0].astype(np.float64)) == 0)[0]) > 0:
        print(
...
def clusterSounds(targetDir, nCluster=-1, descInput=[]):
  ...
  """
    ...
    for cname in dataDetails.keys():
        #iterating over sounds
        for sname in dataDetails[cname].keys():
            l = convFtrDict2List(dataDetails[cname][sname]['feature'])
            attr = l[descInput]
            ftrArr.append(attr)
    ...
    for path, dname, fnames in os.walk(inputDir):
        for fname in fnames:
            if descExt in fname.lower():
                remain, rname, cname, sname = path.split(
                    '\\')[:-3], path.split('\\')[-3], path.split(
                        '\\')[-2], path.split('\\')[-1]
                if cname not in dataDetails:
                    dataDetails[cname] = {}
                fDict = json.load(
                    open(
                        os.path.join('\\'.join(remain), rname, cname, sname,
                                     fname), 'r'))
...
#for i in range(0, 5):
#    for y in range(0, 5):
#        descriptorPairScatterPlot("testDownload\\", descInput=(i, y))

#showDescriptorMapping()
#clusterSounds("testDownload\\", nCluster=3, descInput=[0, 3, 4])
classifySoundkNN("test\\violin\\247402\\247402_6552981-lq.json", "testDownload\\", 3, descInput=[0, 3, 4])
classifySoundkNN("test\\human\\15923\\15923_40665-lq.json", "testDownload\\", 3, descInput=[0, 3, 4])
#C:\Users\n_sto\Documents\sms-tools\assignments\A9\testDownload\violin\247402
