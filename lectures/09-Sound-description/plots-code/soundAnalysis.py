    ...
    for path, dname, fnames in os.walk(inputDir):
        for fname in fnames:
            if descExt in fname.lower():
                ...
                if dataDetails.get(cname, "") == "":
                    dataDetails[cname] = {}
                fDict = json.load(
                    open(os.path.join(rname, cname, sname, fname), 'r'))
...
def plotFeatures(inputDir, descInput=('', ''), anotOn=0):
    ...
    for ii, category in enumerate(dataDetails.keys()):
        ...
        for soundId in dataDetails[category].keys():
            ...
            if descSound.get(descriptors[0], "") == "" or descSound.get(
                    descriptors[1], "") == "":
                print(
                    "Please provide descriptors which are extracted and saved before"
                )
                return -1
...
def clusterSounds(targetDir, nCluster=-1):
    ...
    for cname in dataDetails.keys():
        ...
        for sname in dataDetails[cname].keys():
            ftrArr.append(
                convFtrDict2List(dataDetails[cname][sname]['feature']))
...
clusterSounds(".\freesound-sounds\flute", 2)        
