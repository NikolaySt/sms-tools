...
spectralPeaks = SpectralPeaks(minFrequency=50,
                              maxFrequency=10000,
                              maxPeaks=100,
                              sampleRate=fs,
...
pitchSalienceFunctionPeaks = PitchSalienceFunctionPeaks(minFrequency=100,
                                                        maxFrequency=300)
...
for frame in FrameGenerator(x, frameSize=M, hopSize=H):
    ...
    pitchSalienceFunction_vals = pitchSalienceFunction(peak_frequencies,
                                                       peak_magnitudes)
    salience_peaks_bins_vals, salience_peaks_saliences_vals = pitchSalienceFunctionPeaks(
        pitchSalienceFunction_vals)
