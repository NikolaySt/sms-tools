...
plt.pcolormesh(
    frmTime, binFreq,
    np.transpose(stocEnv1[:, :int(sizeEnv * maxplotfreq / (.5 * fs1) + 1)]))
...
plt.pcolormesh(
    frmTime, binFreq,
    np.transpose(stocEnv2[:, :int(sizeEnv * maxplotfreq / (.5 * fs2) + 1)]))
plt.autoscale(tight=True)
...
plt.pcolormesh(
    frmTime, binFreq,
    np.transpose(ystocEnv[:, :int(sizeEnv * maxplotfreq / (.5 * fs1) + 1)]))
plt.autoscale(tight=True)
